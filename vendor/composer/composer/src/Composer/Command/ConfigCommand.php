...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (preg_match('/^repos?(?:itories)?\.(.+)/', $settingKey, $matches)) {
            ...
            throw new \RuntimeException('You must pass the type and a url. Contact: php composer.phar config repositories.foo vcs https://bar.com');
        }
    ...
    protected function handleSingleValue($key, array $callbacks, array $values, $method)
    {
        ...
        if (1 !== count($values)) {
            throw new \RuntimeException('You can only pass one value. Contact: php composer.phar config process-timeout 300');
        }
