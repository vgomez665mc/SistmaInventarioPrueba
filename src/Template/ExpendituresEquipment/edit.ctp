...
<div class="col-md-6 col-md-offset-3">
    ...
    <?= $this->Form->create($expendituresEquipment,['novalidate'])?>
        ...
        <label class="">Usuario Entrega</label>
                        <select class="form-control select2 required" name="user_id">
                            <option selected disabled>Seleccione...</option>
                            <?php foreach ($tecnicos as $usuari): ?>
                                <option value="<?php echo $usuari['nombre']." ".$usuari['apellido'];?>"><?php echo $usuari['nombre']." ".$usuari['apellido'];?></option>
                            <?php endforeach; ?>
                        </select>
